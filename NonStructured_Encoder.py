...
class NonStructured_Encoder():
    def __init__(self, sess, FLAGS, embed, num_units=None, scope="Sentence_Encoder"):
        ...
        self.num_units = num_units or FLAGS.num_units
        ...
        with tf.variable_scope(scope):
            self._build_embedding(embed)  # fixed build embedding
            self._build_input()
    ...
    def initialize(self, vocab):
        # TODO: potentially np.array to be fixed
        op_in = self.symbol2index.insert(
            tf.constant(vocab), tf.constant(np.array(range(len(vocab)))))
        op_out = self.index2symbol.insert(
            tf.constant(np.array(range(len(vocab)))), tf.constant(vocab))
