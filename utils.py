            ...
            for edu in dialog["edus"]:
                ...
                for i in range(len(text)):
                    if text[i] in "',?.!()\": ":
                        ...
                        if text[i] != " " and (len(tokens) == 0 or tokens[-1] != text[i]):
...
def build_vocab(data):
    ...
    with open(FLAGS.word_vector, "r") as f_in:
        for line in f_in:
            line = line.split()
            # vectors[line[0]] = map(float, line[1:])
            # TODO: causes errors later
            vectors[line[0]] = np.array(list(map(float, line[1:])), dtype=np.float32)
    ...
    for i, word in enumerate(vocab_list):
        if i > FLAGS.vocab_size and word not in vectors:
            continue
    ...
    # it already has float32 type, but there are unrolled map objects
...
def update_buffer(output_feed, input_feed, feed):
    return output_feed + feed[0], {**input_feed, **feed[1]}
    # return output_feed + feed[0], dict(input_feed.items() + feed[1].items())


def init_grad(params):
    return [np.zeros(shape=param.shape) for param in params]
...
def get_batches(data, batch_size, sort=True):
    ...
    while len(data[0]['edus']) == 0:
        data = data[1:]

    batches = []

    # TODO: not sure if range is proper now; but I guess it's satisfied by this bool expression
