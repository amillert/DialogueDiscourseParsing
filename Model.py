...
class Model:
    def __init__(self, sess, FLAGS, embed):  # , data_train=None):
        ...
        self.learning_rate_decay_op = self.learning_rate.assign(self.learning_rate * FLAGS.learning_rate_decay)
        ...
        self.grad_unclipped = [tf.placeholder(tf.float32, param.shape) for param in self.params_all]
    ...
    def update_gradients_buffer(self, o_feed, i_feed, agent, name, attn):
    ...
    def update_gradients_inner(self, agent, grad_hp, grad_hs, g_structured, g_parent, g_current, attn):
    ...
    def backpropagate_hp_flush(self):
        ...
        for attn in range(0, 2):
            ...
            o_feed, i_feed = self.update_gradients_buffer(o_feed, i_feed, self.agent_bi, "bi", attn)
            o_feed, i_feed = self.update_gradients_buffer(o_feed, i_feed, self.agent_multi, "multi", attn)

        res, c = self.sess.run(o_feed, i_feed), 0

        for attn in range(0, 2):
            if len(self.hp_bp_buf[attn]["idx_parent"]) == 0: continue

            self.update_gradients_inner(
    ...
    def build_relation_list(self, batch):
        ...
        for k, dialog in enumerate(batch):
            self.parents.append([[] for _ in range(len(dialog["edus"]))])
            self.parents_relation.append([[] for _ in range(len(dialog["edus"]))])
            self.parents_hp.append([[] for _ in range(len(dialog["edus"]))])
            self.parents_relation_hp.append([[] for _ in range(len(dialog["edus"]))])
            ...
            for j in range(len(dialog["edus"])):
                ...
                for i in range(len(dialog["edus"])):
                    if self.relation_types[k][i][j] > 0 and (i < j and j - i <= self.max_edu_dist):
                        r.append(i)
    ...
    def update_hp_helper(self, hp, _hp, attn):
    ...
    def update_hp(self, batch, fixed_noise=1):
        ...
        def update_hp_buffer(o_feed, i_feed, agent, name, hp, attn):
        ...
        for attn in range(0, 2):
            ...
            o_feed, i_feed = update_hp_buffer(o_feed, i_feed, self.agent_bi, "bi", self.hp_bi, attn)
            o_feed, i_feed = update_hp_buffer(o_feed, i_feed, self.agent_multi, "multi", self.hp_multi, attn)
        ...
        for attn in range(0, 2):
            ...
            self.update_hp_helper(self.hp_bi, res[c], attn)
            if attn:
            ...
            self.update_hp_helper(self.hp_multi, res[c], attn)
            if attn:
    ...
    def clip_gradients(self):
        ...
        if self.use_structured:
            ...
            self.agent_multi.grad_s_encoder_general, gradients_clipped = get_clipped(
                self.agent_multi.grad_s_encoder_general)
    ...
    def initial_setup(self, batch):
        ...
        return cnt_golden, cur, max_edus, unfinished

    def zero_everything(self, unfinished, max_edus):
        ...
        return size, state, state_multi, mask, golden, idx

    def step(self, batch, is_train=False):
        self.is_train = is_train
        sum_loss_bi, cnt_loss_bi = 0, 0
        sum_loss_multi, cnt_loss_multi = 0, 0
        cnt_pred, cnt_cor_bi, cnt_cor_multi = 0, 0, 0,

        cnt_golden, cur, max_edus, unfinished = self.initial_setup(batch)

        while np.sum(unfinished) > 0:
            size, state, state_multi, mask, golden, idx = self.zero_everything(unfinished, max_edus)
            ...
            for k, dialog in enumerate(batch):
                ...
                for i in self.relation_list[idx_j]:
                    if i in self.parents[k][j]: continue
                    golden[idx] = i
                    break  # lol why loop if breaking after first element?; except continue; take until (not)
                idx += 1
            ...
            for k, dialog in enumerate(batch):
                ...
                if action[idx] != len(dialog["edus"]):
                    ...
                    if self.relation_types[k][action[idx]][cur[k][0]]:
                        cnt_cor_bi += 1
                idx += 1
                ...
                if is_train:
                    ...
                    for l, idx in enumerate(idx_multi_train):
                        ...
                        if self.use_shared_encoders:
                            ...
                            self.update_grad_state(
                                batch, self.grad_hs_multi, self.grad_hc_multi, self.grad_hp_multi, g_state[l, :], k, i,
                                j)
                sum_loss_multi += loss
                cnt_loss_multi += 1
        ...
        if is_train and math.isnan(sum_loss_bi) or math.isnan(sum_loss_multi):
